...
import (
	...
	"github.com/go-spring/stdlib/safego"
...
func TestGo(t *testing.T) {
	t.Run("panic recovery", func(t *testing.T) {
		var s string
		safego.Go(t.Context(), func(ctx context.Context) {
	...
	t.Run("context cancel with withoutCancel=false", func(t *testing.T) {
		...
		status := safego.Go(ctx, func(ctx context.Context) {
			<-ctx.Done()
			select {
			case <-ctx.Done():
...
func TestGoValue(t *testing.T) {
	t.Run("panic recovery", func(t *testing.T) {
		s, err := safego.GoValue(t.Context(), func(ctx context.Context) (string, error) {
			panic("something is wrong")
		}, false).Wait()
		assert.That(t, s).Equal("")
	...
	t.Run("successful execution with int", func(t *testing.T) {
		i, err := safego.GoValue(t.Context(), func(ctx context.Context) (int, error) {
			return 42, nil
		}, false).Wait()
	...
	t.Run("multiple goroutines", func(t *testing.T) {
		var arr []*safego.ValueStatus[int]
		for i := range 3 {
			arr = append(arr, safego.GoValue(t.Context(), func(ctx context.Context) (int, error) {
				return i, nil
			}, false))
		}
		for i, g := range arr {
	...
	t.Run("error return", func(t *testing.T) {
		...
		_, err := safego.GoValue(t.Context(), func(ctx context.Context) (string, error) {
			return "", expectedErr
		}, false).Wait()
	...
	t.Run("context cancel with withoutCancel=false", func(t *testing.T) {
		...
		resultFuture := safego.GoValue(ctx, func(ctx context.Context) (string, error) {
			<-ctx.Done()
			select {
			case <-ctx.Done():
	...
	t.Run("context cancel with withoutCancel=true", func(t *testing.T) {
		...
		result, err := safego.GoValue(ctx, func(ctx context.Context) (string, error) {
			select {
			case <-time.After(50 * time.Millisecond):
